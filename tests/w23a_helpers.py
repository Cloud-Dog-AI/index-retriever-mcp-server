...
def _service_endpoint(cfg: dict[str, Any]) -> str:
    for key in ("base_url", "uri", "url", "endpoint"):
        value = _clean_text(cfg.get(key))
        if value:
            return value
    return ""
...
def parser_services_config() -> dict[str, dict[str, Any]]:
    ...
    mineru_env_base_url = _clean_text(os.getenv("MINERU_BASE_URL"))
    if mineru_env_base_url:
        mineru["base_url"] = mineru_env_base_url
    elif not _clean_text(mineru.get("base_url")):
        mineru["base_url"] = _service_endpoint(mineru)

    mineru_base_url = _clean_text(mineru.get("base_url"))
    if not mineru_env_base_url and "mineruapi" in mineru_base_url:
        # Prefer MinerU Gradio endpoint directly; mineruapi /file_parse has been unstable in ST.
        mineru["base_url"] = mineru_base_url.replace("mineruapi", "minerugui")

    mineru_env_api_key = _clean_text(os.getenv("MINERU_API_KEY"))
    if mineru_env_api_key:
        mineru["api_key"] = mineru_env_api_key
    else:
        mineru.setdefault("api_key", _clean_text(mineru.get("api_key")))
    ...
    marker_env_base_url = _clean_text(os.getenv("MARKER_MCP_BASE_URL"))
    if marker_env_base_url:
        marker["base_url"] = marker_env_base_url
    elif not _clean_text(marker.get("base_url")):
        marker["base_url"] = _service_endpoint(marker)

    marker_env_token = _clean_text(os.getenv("MARKER_MCP_AUTH_TOKEN"))
    if marker_env_token:
        marker["auth_token"] = marker_env_token
    else:
        marker.setdefault("auth_token", _clean_text(marker.get("auth_token")))
    ...
    marker.setdefault("timeout_seconds", _parse_float(os.getenv("MARKER_MCP_TIMEOUT_SECONDS"), 1200.0))
    ...
    marker.setdefault("async_max_wait_seconds", _parse_float(os.getenv("MARKER_MCP_ASYNC_MAX_WAIT_SECONDS"), 1200.0))
    marker.setdefault("busy_retry_initial_seconds", _parse_float(os.getenv("MARKER_MCP_BUSY_RETRY_INITIAL_SECONDS"), 35.0))
    marker.setdefault("busy_retry_max_seconds", _parse_float(os.getenv("MARKER_MCP_BUSY_RETRY_MAX_SECONDS"), 1200.0))
    marker.setdefault(
    ...
    marker.setdefault("doc_timeout_seconds", _parse_float(os.getenv("MARKER_MCP_DOC_TIMEOUT_SECONDS"), 1200.0))
    ...
    if transformers_base_url:
        ...
        transformers["base_url"] = _service_endpoint(transformers)
