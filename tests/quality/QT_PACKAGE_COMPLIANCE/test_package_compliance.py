...
class TestPackageCompliance:
    ...
    def test_no_bespoke_logging(self) -> None:
        ...
        assert len(hits) == 0, (
            f"FAIL: {len(hits)} bespoke logging calls found. Replace with cloud_dog_logging:\n"
            + "\n".join(hits[:10])
        )
    ...
    def test_no_bespoke_auth(self) -> None:
        ...
        hits = _grep_count(r"from.*auth|import.*auth", "cloud_dog")
        real_hits: list[str] = []
        for hit in hits:
    ...
    def test_no_direct_llm_calls(self) -> None:
        ...
        hits = _grep_count(r"httpx\.AsyncClient.*ollama|requests\.post.*ollama|openai\.ChatCompletion", "cloud_dog")
        assert len(hits) == 0, (
    ...
    def test_no_hardcoded_secrets(self) -> None:
        ...
        real_hits = [
            ...
            if not any(
                safe in hit.lower()
                for safe in ["test", "example", "placeholder", "changeme", "12345", "os.environ", "config.get"]
            )
        ]
        assert len(real_hits) == 0, (
    ...
    def test_no_internal_hostnames(self) -> None:
        ...
        real_hits = [
            hit
            for hit in hits
            if not any(marker in hit for marker in ["#", '"""', "vault.", "test", "PREPROD", "example", "docs/"])
        ]
        assert len(real_hits) == 0, (
    ...
    def test_ui_dist_exists(self) -> None:
        """PS-30: ui/dist/ must exist for the shipped SPA."""
    ...
    def test_runtime_config_endpoint(self) -> None:
        ...
        has_runtime_config = any(
            "runtime-config" in path.read_text(encoding="utf-8") or "runtime_config" in path.read_text(encoding="utf-8")
