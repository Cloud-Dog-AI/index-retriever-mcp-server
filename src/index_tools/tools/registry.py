...
def build_default_tool_registry() -> ToolRegistry:
    ...
    specs: list[ToolSpec] = [
        ...
        ToolSpec(name="ingest_text", input_model=IngestTextInput, output_model=IngestOutput, description="Ingest inline text content into a profile and collection. Stock MCP clients receive a terminal job result or structured timeout guidance."),
        ToolSpec(name="ingest_text_long", input_model=IngestTextInput, output_model=IngestOutput, description="Ingest larger inline text content with the PS-95 long-call contract. Stock MCP clients receive a terminal job result or structured timeout guidance."),
        ToolSpec(name="ingest_file_async", input_model=IngestFileInput, output_model=IngestOutput, description="Submit file ingestion and return a job ID immediately. Use ingest_file_async_blocking if your MCP client cannot poll job IDs."),
        ToolSpec(name="ingest_file_async_blocking", input_model=IngestFileInput, output_model=IngestOutput, description="Submit file ingestion and wait for a terminal job result or structured timeout guidance for stock MCP clients."),
        ...
        ToolSpec(name="structure_template_match", input_model=GenericToolInput, output_model=GenericToolOutput, description="Score how well a structure document matches a generated template, with matched/missing/extra section breakdown."),
